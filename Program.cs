...
namespace SB.TechnicalTest
{
    class Program
    {
        ...
        /// 
        /// Line 1 start from first floor, drop until marble break.
        /// Line 2 start from top floor, drop until marble not break.
        /// Both line start at the same time, drop until line 1 break, or line 2 not break. 
        ///
        /// </summary>
        /// <returns>Highest safe floor.</returns>
        static int Attempt2()
        {
            var i = 1;
            var j = Building.NumberFloors;
            bool line1Marble;
            bool line2Marble;
            do
            {
                line1Marble = Building.DropMarble(i);
                line2Marble = Building.DropMarble(j);
            }
            while (i++ <= Building.NumberFloors && line1Marble
                    && j-- > 0 && !line2Marble);

            return !line1Marble ? i - 1 : j + 1;
        }
    }
}
